...
struct UnsafeXSubStack : UnsafeStack {
	...
	func fetch<T : PerlSvConvertible>(at index: Int) throws -> T {
		...
		return try T(_fromUnsafeSvPointerCopy: args[index], perl: perl)
	}

	func fetch<T : PerlSvConvertible>(at index: Int) throws -> T? {
		guard index < args.count else { return nil }
		return try Optional<T>(_fromUnsafeSvPointerCopy: args[index], perl: perl)
	}
	...
	func fetchTail<T : PerlSvConvertible>(startingAt index: Int) throws -> [T] {
		...
		for value in args[index..<args.count] {
			tail.append(try T(_fromUnsafeSvPointerCopy: value, perl: perl))
		}
		return tail
	}
	...
	func fetchTail<T : PerlSvConvertible>(startingAt index: Int) throws -> [String: T] {
		...
		while let name = i.next() {
			...
			tail[try String(name, perl: perl)] = try T(_fromUnsafeSvPointerCopy: value, perl: perl)
		}
		return tail
	}
}
